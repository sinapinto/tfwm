...
#define LOAD_CURSORFONT(_c, _char)                                             \
		do {                                                           \
			...
			xcb_create_glyph_cursor(conn, cursors[_c], font, font, \
						_char, (_char + 1), 0, 0, 0,   \
...
#undef LOAD_CURSOR
		...
		cursors[XC_BOTTOM_RIGHT] = xcb_cursor_load_cursor(ctx, "bottom_right_corner");
		cursors[XC_MOVE]         = xcb_cursor_load_cursor(ctx, "fleur");
	}
}

xcb_cursor_t
cursor_get_id(enum a_cursor_t c) {
	if (c >= XC_MAX)
		return XCB_NONE;
	return cursors[c];
}
...
void
cursor_set_window_cursor(xcb_window_t win, enum a_cursor_t c) {
	if (c < XC_MAX)
